...
def test_analysis_counts_segment_distribution_matches_final_claim_breakdown():
    vg = _vg()
    ...
    out = vg._parse_verdict_result(llm, claim, evidence)
    final_statuses = [str(seg.get("status") or "UNKNOWN").upper() for seg in (out.get("claim_breakdown") or [])]
    expected = {}
    for s in final_statuses:
        expected[s] = expected.get(s, 0) + 1
    got = (out.get("analysis_counts") or {}).get("segment_status_distribution", {})
    assert got == expected


def test_human_rationale_is_evidence_balanced_not_template_like():
    vg = _vg()
    claim = "Drinking lemon water detoxifies the liver"
    evidence = [
        {
            "statement": "Lemon juice treatment improved liver injury markers in an alcohol model.",
            "source_url": "https://pmc.ncbi.nlm.nih.gov/articles/PMC5439254/",
            "final_score": 0.56,
            "credibility": 0.7,
        }
    ]
    payload = {
        ...
        "rationale": "test",
        "claim_breakdown": [
            {"claim_segment": claim, "status": "PARTIALLY_VALID", "supporting_fact": evidence[0]["statement"]}
        ],
        "evidence_map": [
            {"evidence_id": 0, "statement": evidence[0]["statement"], "relevance": "NEUTRAL", "relevance_score": 0.38}
        ],
    ...
    assert "contradicts this conclusion" not in r
    assert "evidence" in r.lower()


def test_adaptive_fallback_does_not_promote_when_strict_predicate_floor_blocked():
    vg = _vg()
    claim_breakdown = [
        {
            "claim_segment": "Drinking lemon water detoxifies the liver",
            "status": "UNKNOWN",
            "alignment_debug": {"reason": "strict_predicate_floor_not_met"},
        }
    ]
    adaptive_metrics = {"coverage": 1.0}
    evidence = [
        {
            "statement": "Lemon juice improved liver injury markers in mice.",
            "source_url": "https://pmc.ncbi.nlm.nih.gov/articles/PMC5439254/",
            "semantic_score": 0.82,
            "stance": "neutral",
        }
...
def test_not_all_quantifier_not_supported_by_subset_harm_statement():
    vg = _vg()
    claim = "Not all fats are harmful"
    statement = "Both types of trans fats are harmful to health."
    assert vg._quantifier_support_allowed(claim, statement) is False


def test_prevention_claim_not_matched_by_risk_reduction_only_statement():
    vg = _vg()
    claim = "Omega-3 supplements prevent heart attacks"
    statement = "Omega-3 supplements reduce the risk of heart attacks in some groups."
    assert vg.compute_predicate_match(claim, statement) == 0.0


def test_true_downgraded_when_policy_insufficient_in_binary_enforcer():
    vg = _vg()
    claim = "Generic health action claim"
    payload = {
        "verdict": "TRUE",
        "confidence": 0.8,
        "truthfulness_percent": 80.0,
        "rationale": "test",
        ...
        "evidence_map": [
            {
                "evidence_id": 0,
                "statement": "Some mixed evidence exists.",
                "relevance": "NEUTRAL",
    ...
    out = vg._enforce_binary_verdict_payload(
        claim,
        payload,
        evidence=[{"statement": "Some mixed evidence exists.", "source_url": "https://example.org"}],
    )
    assert out["verdict"] in {"UNVERIFIABLE", "PARTIALLY_TRUE"}
    assert out["verdict"] != "TRUE"
