...
@patch("app.services.vdb.vdb_retrieval.embed_async")
@patch("app.services.vdb.vdb_retrieval.get_pinecone_index")
async def test_vdb_retrieval_without_topics_uses_language_filter_fallback(mock_get_index, mock_embed):
    mock_index = MagicMock()
    mock_get_index.return_value = mock_index
    mock_embed.return_value = [[0.1, 0.2, 0.3]]
        ...
        "matches": [
            {
                "score": 0.78,
                "metadata": {
                    "statement": "Liver and kidneys filter waste products.",
                    "source_url": "https://example.com",
                },
            }
        ]
    ...
    retriever = VDBRetrieval()
    results = await retriever.search("liver kidneys cleanse", top_k=1, topics=[])

    assert len(results) == 1  # nosec
    assert results[0]["statement"].startswith("Liver and kidneys")  # nosec
    call_kwargs = mock_index.query.call_args.kwargs
    assert call_kwargs.get("filter") == {"language": "en"}
