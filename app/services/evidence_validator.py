...
class EvidenceValidator:
    ...
    Logic:
    1. Check if domain is in canonical trusted domain config
    ...
    def get_validation_state(source_url: str) -> ValidationState:
        ...
        # Canonical trusted domain check (exact root or subdomain).
        if is_trusted_domain(domain):
            return ValidationState.TRUSTED

        # Check dynamic admin approvals/rejections.
    ...
    def get_verdict_state(validation_state: ValidationState) -> VerdictState:
        ...
        if validation_state == ValidationState.PENDING_DOMAIN_TRUST:
